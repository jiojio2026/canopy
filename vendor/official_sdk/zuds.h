...
// 填充模式
typedef uint8 ZUDS_FILL_MODE;
#define FILL_MODE_NONE     0    // 不填充
#define FILL_MODE_SHORT    1    // 小于8字节填充至8字节，大于8字节时按DLC就近填充
#define FILL_MODE_MAX      2    // 填充至最大数据长度 (不建议)

// 寻址模式
typedef uint8 ZUDS_ADDRESSING_MODE;
#define ZUDS_ADDRESSING_MODE_NORMAL       0     // 常规寻址
#define ZUDS_ADDRESSING_MODE_EXTENDED     1     // 扩展寻址，数据区第一个字节为目标地址(N_TA)
#define ZUDS_ADDRESSING_MODE_MIXED        2     // 混合寻址，数据区第一个字节为地址扩展(N_AE)

// 传输层设置参数
typedef struct _ZUDS_ISO15765_PARAM
{
    uint8  version;           // 传输协议版本, VERSION_0, VERSION_1
    uint8  max_data_len;      // 单帧最大数据长度, 范围[8, 64], 建议 can:8, canfd:64
    ...
    ZUDS_FILL_MODE  fill_mode;   // 数据长度填充模式, FILL_MODE_NONE, FILL_MODE_SHORT, FILL_MODE_MAX
    ZUDS_ADDRESSING_MODE addressing_mode; // 寻址模式
    uint8 strict_addr_mode;      // 是否严格地址判断，扩展寻址时只接收发给本Tester的数据
    uint8 ecu_addr;              // ECU地址(混合、扩展寻址有效)
    uint8 tester_addr;           // 测试仪地址(混合、扩展寻址有效)
    uint8 extension_addr;        // N_AE(混合寻址有效)
} ZUDS_ISO15765_PARAM;
